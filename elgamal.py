from algebra import mod_inv
from random import randint

PARAM_P = 

PARAM_Q = 0x8CF83642A709A097B447997640129DA299B1A47D1EB3750BA308B0FE64F5FBD3

PARAM_G = 


# call bruteLog with p = PARAM_P and g = PARAM_G

def bruteLog(g, c, p):
    s = 1
    for i in range(p):
        if s == c:
            return i
        s = (s * g) % p
        if s == c:
            return i + 1
    return -1


''' generate a pv key '''


def EG_generate_keys():
    u = randint(1, PARAM_Q - 1)
    U = pow(PARAM_G, u, PARAM_P)

    return (u, U)

# multiplicative version


def EGM_encrypt(m, pubkey):
    r = randint(1, PARAM_P-1)
    (c1, c2) = (pow(PARAM_G, r, PARAM_P), (m * pow(pubkey, r, PARAM_P)))

    return (c1, c2)

# additive version


def EGA_encrypt(m, pubkey):
    r = randint(1, PARAM_Q-1)
    c1 = pow(PARAM_G, r, PARAM_P)
    c2 = (pow(PARAM_G, m, PARAM_P) * pow(pubkey, r, PARAM_P)) % PARAM_P

    return (c1, c2)


def EG_decrypt(c1, c2, pvkey):
    r = pow(c1, pvkey, PARAM_P)
    m = (c2 * mod_inv(r, PARAM_P)) % PARAM_P
    return m
